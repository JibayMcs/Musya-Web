        ...
        <div class="ms_artist_wrapper common_pages_space">
            <div class="album_single_data">
                {{--@php
                    ...
                    if(!empty($artist)){
                        $genre_name = getArtistGenreName(['genre_id' => $artist->artist_genre_id]);
                @endphp--}}
                ...
                <div class="album_single_text">
                    ...
                    <div class="about_artist">{{ $artist->biography }} </div>
                    <div class="album_btn">
                        <a href="#" class="ms_btn play_btn play_music" data-musicid="{{ $artist->id }}" data-musictype="artist" data-url="{{ route('song.play') }}"><span class="play_all"><img
                                    src="{{ asset('assets/images/svg/play_all.svg') }}" alt="">{{ __('frontWords.play_all') }}</span><span class="pause_all"><img
                                    src="{{ asset('assets/images/svg/pause_all.svg') }}" alt="">{{ __('frontWords.pause') }}</span></a>
                        <a href="javascript:void(0);" class="ms_btn add_to_queue" data-musicid="{{ $artist->id }}" data-musictype="artist"><span class="play_all"><img
                                    src="{{ asset('assets/images/svg/add_q.svg') }}" alt="">{{ __('frontWords.add_to_queue') }}</span></a>
                    </div>
                </div>
                        ...
                        <li class="list_more">
                            ...
                            <ul class="ms_common_dropdown ms_downlod_list list_more">
                                ...
                                <li class="favourite_icon">
                                    <a href="javascript:void(0);" class="addToFavourite" data-favourite="{{ $artist->id }}" data-type="artist">
                                        {{--@if($favourite == 1)
                                            <svg width='19px' height='19px' id='Layer_1' data-name='Layer 1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 391.8 391.8'>
                                                <defs>
                                                    <style>.cls-1 {
                                                            fill: #d80027;
                                                        }</style>
                                                ...
                                                <path class='cls-1'
                                                      d='M280.6,43.8A101.66,101.66,0,0,1,381.7,144.9c0,102-185.8,203.1-185.8,203.1S10.2,245.5,10.2,144.9A101.08,101.08,0,0,1,111.3,43.8h0A99.84,99.84,0,0,1,196,89.4,101.12,101.12,0,0,1,280.6,43.8Z'></path>
                                            </svg>
                                        @else--}}
                                ...
                                {{--<li>
                                    @if(!empty($userPlan) && $userPlan->is_download == 1)
                                        <a href="javascript:void(0);" class="download_list" data-musicid="{{ $artist->id }}" data-type="artist">
                                            @else
                                ...
                                <li>
                                    <a href="javascript:void(0);" class="add_to_queue" data-musicid="{{ $artist->id }}" data-musictype="artist">
                                        <span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span>
                                        {{ __("frontWords.add_to_queue") }}
                                    </a>
                                </li>
                                <li>
                                    <a href="javascript:void(0);" class="ms_share_music" data-shareuri="{{url('artists/' . $artist->id)}}"
                                       data-sharename="{{ $artist->name }}">
            ...
            <div class="album_list_wrapper">
                                ...
                                if(!empty($audioDetail)){
                                    foreach($audioDetail as $audios){
                                        ...
                                        foreach($getArtist as $artistName){
                                            $artists = select(['column'=>'name','table'=>'artists','where'=>['id'=>$artistName] ]);
                                            if(count($artists) > 0){
                                                $name .= $artists[0]->name.',';
                                            }
                                        }
                ...
                @foreach($artist->albums as $album)
                    @for($i = 0; $i < $album->tracks->count(); $i++)
                        <ul class="play_music" data-musicid="{{ $album->tracks[$i]->id }}" data-musictype="audio" data-url="{{ route('song.play') }}">
                            <li>
                                <span class="play_no">{{ $i+1 }}</span>
                            ...
                            <li><a href="javascript:void(0)">{{ $album->tracks[$i]->name }}</a></li>
                            <li>{{ rtrim($album->tracks[$i]->album->name,',') }}</li>
                            <li class="text-center">{{ $album->tracks[$i]->duration() }}</li>
                            ...
                            <li class="list_more">
                                ...
                                <ul class="ms_common_dropdown ms_downlod_list">
                                    ...
                                    @endphp--}}
                                    <li>
                                        <a href="javascript:void(0);" class="add_to_queue" data-musicid="{{ $album->tracks[$i]->id }}" data-musictype="audio">
                                            <span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span>
                                            {{ __("frontWords.add_to_queue") }}
                                        </a>
                                    </li>
                                    <li>
                                        <a href="javascript:void(0);" class="ms_add_playlist" data-musicid="{{ $album->tracks[$i]->id }}">
                                            <span class="common_drop_icon drop_playlist"></span>{{ __('frontWords.add_to_playlist') }}
                                        </a>
                                    </li>
                                    <li>
                                        <a href="javascript:void(0);" class="ms_share_music" data-shareuri="audio_slug"
                                           data-sharename="{{ $album->tracks[$i]->name }}">
                                            <span class="common_drop_icon drop_share"></span>{{ __('frontWords.share') }}
                                        </a>
                                    </li>
                                </ul>
